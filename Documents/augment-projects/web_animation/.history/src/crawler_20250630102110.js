...
class WebsiteCrawler {
  ...
  async discoverRoutes() {
    console.log(chalk.yellow('🔍 開始發現網站路由...'));
    ...
    try {
      ...
      console.log(chalk.green(`✅ 發現 ${this.discoveredUrls.size} 個有效路由`));
      return Array.from(this.discoveredUrls);
      
    } catch (error) {
      console.error(chalk.red('❌ 路由發現失敗:'), error.message);
      throw error;
    }
  }
  ...
  async crawlPage(url) {
    ...
    console.log(chalk.cyan(`📄 正在爬取: ${url}`));
    ...
    try {
      ...
      console.error(chalk.red(`❌ 爬取失敗 ${url}:`), error.message);
      return null;
    }
  }
  ...
  async crawlAllPages() {
    console.log(chalk.blue('🕷️  開始全站爬取...'));
    ...
    console.log(chalk.green(`✅ 爬取完成！共處理 ${results.length} 個頁面`));
    return results;
  }
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      console.error(chalk.red('爬取過程發生錯誤:'), error);
    } finally {
      await crawler.close();
    }
  })();
